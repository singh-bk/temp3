...
class WindowProcessorImpl implements WindowProcessor {
    ...
    public void put(String key, int value) {
        ...
        if (node != null) {
            node.ttl = Instant.now().toEpochMilli() + windowSize;
            node.value = value;
            map.put(key, node);
            ...
            node = new Node(key, value, Instant.now().toEpochMilli() + windowSize);
            map.put(key, node);
            if (head == null) {
                head = node;
                tail = node;
    ...
    public Integer get(String key) {
        ...
        if (node != null && node.ttl > Instant.now().toEpochMilli()) {
            return node.value;
        }
        return null;
    }
    ...
    public double getAverage() {
        ...
        long total = 0l;
        while (tail.ttl < Instant.now().toEpochMilli()) {
...
class Node {
    ...
    public Node(String key, int value, long ttl) {
        this.key = key;
        this.value = value;
        this.ttl = ttl;
    }
